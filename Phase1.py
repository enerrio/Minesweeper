...
def convert_click_to_row_column(c_point, rows, columns):
    for i in range(rows):
        ...
        for j in range(columns):
            ...
            if top_left.getX() <= c_point.getX() and bottom_right.getX() >= c_point.getX() and \
                            top_left.getY() <= c_point.getY() and bottom_right.getY() >= c_point.getY():
                    return i, j
    return None, None
...
def expose_empty_cells(game_board_markers, game_board_images, row, column):
    ...
    for i in range(rows):
        if row > 0 and game_board_markers[row-i][column] == 0:
            game_board_images[row-i][column].undraw()
        if row < rows - i and game_board_markers[row + i][column] == 0:
            game_board_images[row+i][column].undraw()
        else:
            break
    for i in range(columns):
        if column > 0 and game_board_markers[row][column-i] == 0:
            game_board_images[row][column-i].undraw()
        if column < columns - i and game_board_markers[row][column+i] == 0:
            game_board_images[row][column+i].undraw()
        else:
            break
    for i in range(rows):
        for j in range(columns):
            if row > 0 and column > 0 and game_board_markers[row-i][column-j] == 0:
                game_board_images[row-i][column-j].undraw()
            if row > 0 and column < columns-j and game_board_markers[row-i][column+j] == 0:
                game_board_images[row-i][column+j].undraw()
            if row < rows-i and column > 0 and game_board_markers[row+i][column-j] == 0:
                game_board_images[row+i][column-j].undraw()
            if row < rows-i and column < columns-j and game_board_markers[row+i][column+j] == 0:
                game_board_images[row+i][column+j].undraw()
            else:
                break
   ...
   #     game_board_images[row+1][column].undraw()                  #1st Iteration, useless with above for loop
   ...
   #     game_board_images[row+1][column+1].undraw()
    ...
    # if row > 0 and game_board_markers[row-2][column] == 0:
    #     game_board_images[row-2][column].undraw()
    # if row < rows-2 and game_board_markers[row+2][column] == 0:
    #     game_board_images[row+2][column].undraw()
    # if column > 0 and game_board_markers[row][column-2] == 0:
    #     game_board_images[row][column-2].undraw()
    # if column < columns-2 and game_board_markers[row][column+2] == 0:     # 2nd Iteration
    #     game_board_images[row][column+2].undraw()
    # if row > 0 and column > 0 and game_board_markers[row-2][column-2] == 0:
    #     game_board_images[row-2][column-2].undraw()
    # if row > 0 and column < columns-2 and game_board_markers[row-2][column+2] == 0:
    #     game_board_images[row-2][column+2].undraw()
    # if row < rows-2 and column > 0 and game_board_markers[row+2][column-2] == 0:
    #     game_board_images[row+2][column-2].undraw()
    # if row < rows-2 and column < columns-2 and game_board_markers[row+2][column+2] == 0:
    #     game_board_images[row+2][column+2].undraw()
